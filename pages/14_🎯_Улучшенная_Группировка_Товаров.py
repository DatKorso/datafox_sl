...
from utils.advanced_grouping_ui_components import (
    ...
    render_help_section,
    render_marketplace_summary_table
)
...
**Новое**: 
- 📸 Теперь в таблицах отображаются фотографии товаров!
- 📋 **Добавлена итоговая таблица для загрузки на маркетплейс** со структурой:
  - WB SKU (артикул ВБ)
  - OZ Vendor Code (артикул поставщика на Озоне)
  - Объединяющий код (для группировки товаров)
""")
...
with st.sidebar:
    ...
    **Основные улучшения:**
    ...
    - 🚀 **Итоговая таблица для загрузки на маркетплейс**
    """)
...
# Основной интерфейс
tab1, tab2, tab3 = st.tabs(["🎯 Группировка", "📊 Результаты", "📋 Итоговая таблица"])
...
with tab3:
    ...
    if 'grouping_result' not in st.session_state:
        ...
        # Отображаем итоговую таблицу для загрузки на маркетплейс
        render_marketplace_summary_table(result, conn)

# Футер страницы
st.divider()
st.markdown("""
<div style='text-align: center; color: #666; font-size: 0.8em;'>
    DataFox SL - Улучшенная группировка товаров v2.2.0<br>
    Исправленная версия с корректной компенсацией рейтинга + итоговая таблица для маркетплейса
</div>
