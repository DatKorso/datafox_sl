...
- **PHOTO_FROM_WB**: Изображения товаров с WB (NEW!)
  - Автоматически загружает и вставляет изображения товаров из Wildberries
  - Создает гиперссылку на страницу товара на WB
    ...
    if df is not None:
        ...
        with st.expander("📝 Колонки, которые будут заполнены", expanded=False):
            ...
            if 'PHOTO_FROM_WB' in df.columns:
                expected_cols.append("**🖼️ Изображения товаров**: PHOTO_FROM_WB - автоматическая загрузка изображений с WB")
            ...
            if process_button:
                ...
                with st.spinner("🔄 Обработка отчета... Это может занять некоторое время..."):
                    ...
                    with progress_container:
                        ...
                        # Process the report
                        success, error_msg, stats = process_analytic_report(conn, selected_file_path)
...
with st.expander("❓ Справка и примеры"):
    ...
    5. **Загрузка изображений**: Для колонки PHOTO_FROM_WB загружаются изображения товаров с WB
    ...
    ### Функция изображений PHOTO_FROM_WB:
    - **Автоматическая загрузка**: Система автоматически генерирует URL изображения на основе WB_SKU
    ...
    - **Резервное содержимое**: Если изображение не загружается, ячейка остается пустой
    ...
    - Загрузка изображений может занимать дополнительное время
