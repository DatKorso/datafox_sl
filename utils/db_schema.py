...
HARDCODED_SCHEMA = {
    "oz_orders": {
        ...
        "read_params": {
            ...
            'header': 0,
            'dtype': {
                'OZON id': 'str'  # Читаем как строку, чтобы избежать overflow
            }
        }, # For import page
        ...
        "columns": [
            ...
            {'target_col_name': 'oz_sku',             'sql_type': 'BIGINT',  'source_col_name': 'OZON id', 'notes': 'convert_to_bigint'},
    ...
    "oz_products": {
        ...
        "read_params": {
            ...
            'header': 0,
            'dtype': {
                'Ozon Product ID': 'str',  # Читаем как строку, чтобы избежать overflow
                'SKU': 'str'  # SKU тоже может быть большим числом
            }
        },
        ...
        "columns": [
            ...
            {'target_col_name': 'oz_product_id',      'sql_type': 'BIGINT',  'source_col_name': 'Ozon Product ID', 'notes': 'convert_to_bigint'},
            {'target_col_name': 'oz_sku',             'sql_type': 'BIGINT',  'source_col_name': 'SKU', 'notes': 'convert_to_bigint'},
    ...
    "oz_barcodes": {
        ...
        "read_params": {
            ...
            'dtype': {
                'Артикул': 'str',
                'Ozon Product ID': 'str',  # Читаем как строку, чтобы избежать overflow
                'Штрихкод': 'str'  # Штрихкоды тоже всегда должны быть строками
            }
        },
    ...
    "wb_products": {
        ...
        "read_params": {
            ...
            'skip_rows_after_header': 1,
            'dtype': {
                'Артикул WB': 'str'  # Читаем как строку, чтобы избежать overflow
            }
        },
        "config_report_key": "wb_products_dir",
        "columns": [
            {'target_col_name': 'wb_sku',      'sql_type': 'BIGINT', 'source_col_name': 'Артикул WB', 'notes': 'convert_to_bigint'},
    ...
    "wb_prices": {
        ...
        "file_type": "xlsx",
        "read_params": {
            'sheet_name': "Отчет - цены и скидки на товары", 
            'header': 0,
            'dtype': {
                'Артикул WB': 'str'  # Читаем как строку, чтобы избежать overflow
            }
    ...
    "oz_card_rating": {
        ...
        "read_params": {
            ...
            'decimal': ',',
            'dtype': {
                'RezonitemID': 'str'  # Читаем как строку, чтобы избежать overflow
            }
        },
        "config_report_key": "oz_card_rating_xlsx",
        "columns": [
            {'target_col_name': 'oz_sku', 'sql_type': 'BIGINT', 'source_col_name': 'RezonitemID', 'notes': 'convert_to_bigint'},
            {'target_col_name': 'oz_vendor_code', 'sql_type': 'VARCHAR', 'source_col_name': 'Артикул', 'notes': None},
